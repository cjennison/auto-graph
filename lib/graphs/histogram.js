AutoGraph.graphs.histogram = {
	init:function(data){
		...
		var form = $('<form><label><input type="radio" name="mode" value="grouped" checked> Grouped</label> <label><input type="radio" name="mode" value="stacked"> Stacked</label></form>')
...
Histogram.prototype.initGraph = function(cb){
	...
	var xLinear = d3.scale.linear()
					.domain([d3.max(this.totalData, function(d){return Number(d['x'])}), d3.min(this.totalData, function(d){return Number(d['x'])})])
					.range([width, 0]);
